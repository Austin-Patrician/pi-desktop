...
import type { ExtensionAPI, ExtensionContext } from "../extensions/index.ts";
...
const PLAN_BLOCKED_EXTENSION_TOOLS = new Set<string>(["subagent"]);
const PLAN_MANAGED_TOOLS = new Set<string>([...PLAN_MODE_TOOLS, ...NORMAL_MODE_TOOLS, ...PLAN_BLOCKED_EXTENSION_TOOLS]);
	...
	function getPlanModeTools(activeToolNames: string[]): string[] {
		return uniqueToolNames([
			...activeToolNames.filter(
				(name) => !PLAN_MODE_DISABLED_TOOLS.has(name) && !PLAN_BLOCKED_EXTENSION_TOOLS.has(name),
			),
			...PLAN_MODE_TOOLS,
		]);
	}
	...
	pi.registerCommand("todos", {
		...
		handler: async (_args, ctx) => {
			...
			const list = todoItems
				.map((item, index) => `${index + 1}. ${item.completed ? "x" : "o"} ${item.text}`)
				.join("\n");
	...
	pi.on("context", async (event) => {
		...
		return {
			messages: event.messages.filter((message) => {
				const typedMessage = message as AgentMessage & { customType?: string };
				if (typedMessage.customType === "plan-mode-context") return false;
				if (typedMessage.role !== "user") return true;
				...
				if (Array.isArray(content)) {
					return !content.some(
						(part) => part.type === "text" && (part as TextContent).text?.includes("[PLAN MODE ACTIVE]"),
					);
				}
				return true;
			}),
