...
import { withFileMutationQueue } from "../tools/index.ts";
import { type BuiltinSubagentAgentDefinition, getBuiltinSubagentAgentDefinitions } from "./subagent-assets.ts";
...
function getBuiltinAgents(): AgentConfig[] {
	return getBuiltinSubagentAgentDefinitions().map((definition: BuiltinSubagentAgentDefinition) => ({
...
function getPiInvocation(args: string[]): { command: string; args: string[] } {
	...
	if (
		...
		(currentScript === srcCliPath || currentScript === distCliPath)
	) {
		return { command: process.execPath, args: [...process.execArgv, currentScript, ...args] };
	}

	if (fs.existsSync(distCliPath)) {
		return { command: process.execPath, args: [distCliPath, ...args] };
	}

	if (fs.existsSync(srcCliPath)) {
		return { command: process.execPath, args: [srcCliPath, ...args] };
	}
...
export default function subagentExtension(pi: ExtensionAPI): void {
	pi.registerTool({
		...
		description: [
			...
			`Default agent scope is "user" (from ${path.join(getAgentDir(), "agents")}) and includes built-in agents.`,
			...
			if (params.chain && params.chain.length > 0) {
				...
				for (let index = 0; index < params.chain.length; index++) {
					...
					if (isFailedResult(result)) {
						...
						return {
							content: [
								{ type: "text", text: `Chain stopped at step ${index + 1} (${step.agent}): ${errorMsg}` },
							],
		...
		renderResult(result, { expanded }, theme) {
			...
			if (details.mode === "single" && details.results.length === 1) {
				...
				const error = isFailedResult(single);
				...
				if (expanded) {
					...
					if (error && single.stopReason) header += ` ${theme.fg("error", `[${single.stopReason}]`)}`;
					container.addChild(new Text(header, 0, 0));
					if (error && single.errorMessage)
				...
				if (error && single.stopReason) text += ` ${theme.fg("error", `[${single.stopReason}]`)}`;
				if (error && single.errorMessage) text += `\n${theme.fg("error", `Error: ${single.errorMessage}`)}`;
						...
						for (const displayItem of displayItems) {
							if (displayItem.type === "toolCall") {
								container.addChild(
									new Text(
										theme.fg("muted", "→ ") +
											formatToolCall(displayItem.name, displayItem.args, theme.fg.bind(theme)),
										0,
										0,
									),
								);
